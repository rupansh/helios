...
enum HeliosTranslatorStatusValues {
    ...
    HELIOS_TRANSLATOR_STATUS_MAX = 38
};
...
/* Records this interface REFERENCES rather than restates. Their C mirror is a
 * sibling header (the HQA1/HTS1 projection of
 * protocol/src/translation_session.rs); these forward declarations keep this
 * header self-contained and typed, and a consumer that needs the field layout
 * includes that header too. The byte sizes below are asserted against the Rust
 * records at compile time on the Rust side. */
...
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosSyncProgressJoinV1, reserved) == 16, "");
 ...
 * and the two session facts the UMD needs in order to encode HOB1/HOS1.
 *
...
typedef struct HeliosTranslatorInstanceV1 {
    ...
    /* 24: the nonzero HTS1 session generation INIT returned. The UMD needs it
     * for HOB1 offset 16 and HOS1 offset 16. A generation counter, not a key:
     * no lookup uses it (invariant 10). */
    uint64_t session_generation;
    uint32_t endpoint_capacity;  /* 32 granted; <= requested, nonzero */
    /* 36: echoed RECORD_ONLY. A cross-check, not the tag — §13.2 (3359) requires
...
HELIOS_TRANSLATOR_STATIC_ASSERT(sizeof(HeliosTranslatorInstanceV1) == 40,
                                "HeliosTranslatorInstanceV1 must be 40 bytes");
...
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosTranslatorInstanceV1, session_generation) == 24, "");
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosTranslatorInstanceV1, endpoint_capacity) == 32, "");
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosTranslatorInstanceV1, submission_mode) == 36, "");
...
typedef struct HeliosQueueAttachRequestV1 {
    ...
    /* 20: exactly one HQA1 context-kind flag (D3D11 physical / D3D12 virtual). */
    uint32_t context_flags;
} HeliosQueueAttachRequestV1;

HELIOS_TRANSLATOR_STATIC_ASSERT(sizeof(HeliosQueueAttachRequestV1) == 24,
                                "HeliosQueueAttachRequestV1 must be 24 bytes");
...
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosQueueAttachRequestV1, context_flags) == 20, "");
 ...
 * the ICD can produce.
 *
 * Field-for-field parallel to HOB1's HeliosOuterBatchUseV1 (helios_wddm.h) —
 * same 40 bytes, same access_flags, same {first_operand, operand_count} run —
 * differing in exactly one place: where HOB1 has {address_or_index,
 * expected_allocation_generation}, which are WDDM identity, this has
 * {outer_allocation_token, byte_offset}, which are not.
 *
...
typedef struct HeliosSealedResourceUseV1 {
    ...
    /* 8: byte offset of the used range within that allocation. MUST BE ZERO on a
     * D3D11 physical context — HOB1's identity_kind 1 arm is an allocation-list
     * index with no room for a sub-allocation offset, so a nonzero value there
     * is HELIOS_TRANSLATOR_STATUS_D3D11_SUBRANGE_USE at seal time. */
    uint64_t byte_offset;
    uint64_t byte_length;    /* 16 nonzero */
    ...
    uint16_t operand_count;  /* 28 how many operands belong to this use */
    uint16_t reserved0;      /* 30 zero */
...
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosSealedResourceUseV1, byte_offset) == 8, "");
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosSealedResourceUseV1, byte_length) == 16, "");
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosSealedResourceUseV1, access_flags) == 24, "");
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosSealedResourceUseV1, operand_count) == 28, "");
HELIOS_TRANSLATOR_STATIC_ASSERT(offsetof(HeliosSealedResourceUseV1, reserved0) == 30, "");
...
typedef struct HeliosTranslatorRefusalCountersV1 {
    ...
    uint64_t loader_provenance_rejected;       /* 56 §13.2 3357-3358 */
...
HELIOS_TRANSLATOR_STATIC_ASSERT(sizeof(HeliosTranslatorRefusalCountersV1) == 88,
                                "HeliosTranslatorRefusalCountersV1 must be 88 bytes");
