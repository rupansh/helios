//! ⛔ **PRE-RETIREMENT System-class / present-ticket WDDM ABI — quarantined.**
//!
//! This module is the exact pre-"HPS2 retirement" contents of `wddm.rs`
//! (everything [`crate::wddm`] no longer defines after that rewrite,
//! commit `e8bafed`), copied here verbatim byte-for-byte, layout and all.
//!
//! **`docs/HELIOS_PRESENT_SYNC_RETIREMENT.md` section 17.1 deletes every record
//! in this file outright.** It survives here, and ONLY here, because
//! `kmd_render`, `umd`, and `umd12` still call it and cannot be made to compile
//! without it — this module is the bridge, not a reprieve. It exists *solely*
//! so those three crates keep building while each lane migrates its own call
//! sites, on its own schedule, to the replacement ABI in [`crate::wddm`].
//!
//! ⛔⛔ **It must NOT gain a new caller, a new symbol, or a new field.** Nothing
//! here is a target for enhancement, refactor, or reuse in new code — a wire
//! shape frozen here is frozen because a deployed driver still speaks it, not
//! because it is a good shape. Treat every edit to this file with suspicion;
//! the only legitimate edit is deleting a symbol once its last caller is gone,
//! and eventually deleting the whole file once section 17.1 is fully satisfied.
//!
//! # What replaced what
//!
//! | Retired here | Replacement in [`crate::wddm`] |
//! |---|---|
...
//! identity construct), so they were never removed from `wddm.rs` and are
//! imported from there below rather than redefined here.
//!
//! # Provenance
//!
//! Copied verbatim from `wddm.rs` as of commit `0799028` (the last commit
//! before the HPS2-retirement rewrite in `e8bafed`) — `git show
//! 0799028:protocol/src/wddm.rs`. Every struct, constant, enum, impl, free
//! function, doc comment, `#[derive]`, `#[repr(C)]`, and compile-time size/
//! offset assertion below is unchanged from that revision; only this header
//! and the `D3DDDIFMT_*` import (see above) are new.

use bytemuck::{Pod, Zeroable};

// `D3DDDIFMT_A8R8G8B8` / `D3DDDIFMT_X8R8G8B8` were never removed from
// `wddm.rs` (see the module doc above), so they are imported rather than
// redefined here. A private `use` (not `pub use`) so `pub use wddm::*; pub
// use wddm_legacy::*;` in lib.rs cannot see two definitions of either name.
use crate::wddm::{D3DDDIFMT_A8R8G8B8, D3DDDIFMT_X8R8G8B8};
...
//
// `D3DDDIFMT_A8R8G8B8` / `D3DDDIFMT_X8R8G8B8` are NOT redefined here: they
// already live in `crate::wddm` (the new module retained them verbatim,
// unrelated to the HPS2 retirement) and are imported at the top of this file.
// Redefining them here would make `pub use wddm::*; pub use wddm_legacy::*;`
// in lib.rs an ambiguous glob re-export.
