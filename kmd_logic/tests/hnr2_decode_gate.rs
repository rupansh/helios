...
//!
//! # ⚠ Two defeats this gate does NOT catch, measured rather than assumed
//!
//! * **Substituting the advertised `HELIOS_HVC1_DMA_BUFFER_BYTES` constant for
//!   the `DmaSize` dxgkrnl actually returned.** Every corpus batch has
//!   `DmaSize == the constant`, so the substitution is invisible. Closing it
//!   needs a corpus case with `DmaSize < CommandLength`; until then the
//!   `CommandLengthAboveDmaBuffer` arm is unexercised here.
//! * **Two contexts sharing one assembler.** The corpus is a single context with
//!   one monotone token stream. Making the assembler a process-global *is*
//!   caught (the caller's `RenderContext` then never moves, and this test reads
//!   it), but a genuinely shared assembler across two interleaved contexts is
//!   not. Closing it needs a second context id in the corpus.
