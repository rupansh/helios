...
| A SupportsCpuHostAperture segment must be the LAST reported segment | AddAdapter Code 43 (ETW-proven 2026-07-05) |
| ⚠ The second half of that row — "classic CpuVisible memory segments are rejected" — was **FALSIFIED 2026-08-10** and must not be relied on. `BarSegFlags=0x02` (CpuVisible=1, SupportsCpuHostAperture=0, CacheCoherent=0) starts `OK / CM_PROB_NONE` on build 26100.8875 and `helios_paintcap` shows a fully composited live desktop. `BarF` moved 28→2→28, so the knob provably took effect. Bound: that is the current segment table with the BAR segment's flags changed, **not** the retirement's exact two-segment HLM1 profile, and the driver still registers the CpuHostAperture callbacks that `§17.6` deletes | the shape `HELIOS_PRESENT_SYNC_RETIREMENT.md` §10.7 requires of HLM1 is admitted, so the KMD memory lane is not blocked |
| ⛔ **Admitted is not aliased** (`docs/retirement/FINDINGS.md` **F16**, 2026-08-11, nine configurations on KMD 22.22.276.0). The HLM1 flag shape boots and the pool really is placed in HLM1 — and an HVM1 allocation's `D3DKMTLock2` view is STILL not its venus blob, in every combination of `BarSegFlags` 0x1C/0x02/0x06, `AccessedPhysically` on/off, and BAR eligibility. `D3DKMTLock2` on this build is a copy protocol: VidMm moves HLM1 content by paging transfer and hands the CPU the system backing. ⛔ And `Hlm1Only=1` (dropping the aperture from the supported set) does not redirect the placement, it BREAKS the page-in — dxgkrnl's ETW: "WORKER_THREAD: Unrecoverable page in failure" | the aliased CPU view has to come from `HELIOS_ESCAPE_MAP_BLOB`, which already returns one, not from §10.7's Lock2 model |
