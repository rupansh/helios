...
//
// GUID_WDDM_INTERFACE_PANEL_BUFFER_CONTROL {7D1788D3-92DB-4E8B-8E85-6CC56D87F1CC}
//
DEFINE_GUID(GUID_WDDM_INTERFACE_PANEL_BUFFER_CONTROL, 0x7d1788d3, 0x92db, 0x4e8b, 0x8e, 0x85, 0x6c, 0xc5, 0x6d, 0x87, 0xf1, 0xcc);
...
#define DXGKDDI_FEATURE_INTERFACE_VERSION_1 0x1
#define DXGKDDI_FEATURE_INTERFACE_VERSION_2 0x2
...
typedef struct _DXGKDDI_FEATURE_INTERFACE_2
{
    USHORT Size;
    USHORT Version;
    PVOID Context;
    ...
    PDXGKDDI_QUERYFEATURESUPPORT QueryFeatureSupport;
    PDXGKDDI_QUERYFEATUREINTERFACE QueryFeatureInterface;
    PDXGKDDI_QUERYFEATURESUPPORT2 QueryFeatureSupport2;
} DXGKDDI_FEATURE_INTERFACE_2, *PDXGKDDI_FEATURE_INTERFACE_2;
...
//
// Panel Buffering Control Interface
//

#define DXGK_PANEL_BUFFER_CONTROL_INTERFACE_VERSION_1 0x01

typedef enum _DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG
{
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_NONE = 0,

    // Generic support
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_FULLSCREEN_UDPATES = 1 << 0,
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_PARTIAL_UPDATES = 1 << 1,

    // Generic buffering-across-mode-change support
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_ACROSS_PIXEL_CLOCK_CHANGE = 1 << 2,
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_ACROSS_RESOLUTION_CHANGE = 1 << 3,
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_ACROSS_COLOR_FORMAT_CHANGE = 1 << 4,

    // Hardware-specific implementations
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_DISPLAYPORT_PSR = 1 << 16,
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_DISPLAYPORT_REPLAY = 1 << 17,
    DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG_MIPI_COMMAND_MODE = 1 << 18,
} DXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG, *PDXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG;

// Queries panel buffering capability flags for a given target
typedef
_Function_class_DXGK_(DXGKDDI_PANEL_BUFFER_CONTROL_GET_CAPABILITIES)
_IRQL_requires_DXGK_(PASSIVE_LEVEL)
NTSTATUS
(*DXGKDDI_PANEL_BUFFER_CONTROL_GET_CAPABILITIES)(
    _In_ PVOID MiniportDeviceContext,
    _In_ D3DDDI_VIDEO_PRESENT_TARGET_ID VidPnTargetId,
    _Out_ PDXGK_PANEL_BUFFER_CONTROL_CAPABILITY_FLAG pCapabilities
    );

typedef enum _DXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE
{
    DXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE_NONE = 0,
    DXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE_HOLD_FRAME = 1,
    DXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE_HOLD_FRAME_FOR_MODE_CHANGE = 2,
} DXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE, *PDXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE;

// Receives requests from the OS to enter/exit panel buffering
typedef
_Function_class_DXGK_(DXGKDDI_PANEL_BUFFER_CONTROL_SET_OVERRIDE_STATE)
_IRQL_requires_DXGK_(PASSIVE_LEVEL)
NTSTATUS
(*DXGKDDI_PANEL_BUFFER_CONTROL_SET_OVERRIDE_STATE)(
    _In_ PVOID MiniportDeviceContext,
    D3DDDI_VIDEO_PRESENT_TARGET_ID VidPnTargetId,
    DXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE OverrideState,
    _In_ PKEVENT CompletedEvent
    );

// Called after DXGKDDI_PANEL_BUFFER_CONTROL_SET_OVERRIDE_STATE raises CompletedEvent to get the result of the operation
typedef
_Function_class_DXGK_(DXGKDDI_PANEL_BUFFER_CONTROL_SET_OVERRIDE_STATE)
_IRQL_requires_DXGK_(PASSIVE_LEVEL)
NTSTATUS
(*DXGKDDI_PANEL_BUFFER_CONTROL_GET_OVERRIDE_RESULT)(
    _In_ PVOID MiniportDeviceContext,
    D3DDDI_VIDEO_PRESENT_TARGET_ID VidPnTargetId,
    _Out_ NTSTATUS *SetOverrideStateStatus,
    _Out_ DXGK_PANEL_BUFFER_CONTROL_OVERRIDE_STATE *CurrentOverrideState
    );

// Describes OS policy on how a driver is allowed to enter/exit panel buffering automatically
typedef enum _DXGK_PANEL_BUFFER_CONTROL_AUTOMATIC_POLICY
{
    // The driver may enter/exit panel buffering automatically as per their own policy
    DXGK_PANEL_BUFFER_CONTROL_AUTOMATIC_POLICY_DEFAULT,
    // The driver may enter panel buffering automatically but may not utilize partial updates (the screen must be updated fully for any new content)
    DXGK_PANEL_BUFFER_CONTROL_AUTOMATIC_POLICY_FULLSCREEN_ONLY,
    // The driver may never enter panel buffering on this panel while this policy is set (generally for debugging/troubleshooting panel buffering issues)
    DXGK_PANEL_BUFFER_CONTROL_AUTOMATIC_POLICY_NEVER,
} DXGK_PANEL_BUFFER_CONTROL_AUTOMATIC_POLICY, *PDXGK_PANEL_BUFFER_CONTROL_AUTOMATIC_POLICY;

// Sets a driver's automatic panel buffering policy.
typedef
_Function_class_DXGK_(DXGKDDI_PANEL_BUFFER_CONTROL_SET_AUTOMATIC_POLICY)
_IRQL_requires_DXGK_(PASSIVE_LEVEL)
NTSTATUS
(*DXGKDDI_PANEL_BUFFER_CONTROL_SET_AUTOMATIC_POLICY)(
    _In_ PVOID MiniportDeviceContext,
    _In_ D3DDDI_VIDEO_PRESENT_TARGET_ID VidPnTargetId,
    DXGK_PANEL_BUFFER_CONTROL_AUTOMATIC_POLICY Policy
    );

typedef struct _DXGK_PANEL_BUFFER_CONTROL_INTERFACE_1 {
    USHORT Size;
    USHORT Version;
    PVOID Context;
    PINTERFACE_REFERENCE InterfaceReference;
    ...
    DXGKDDI_PANEL_BUFFER_CONTROL_GET_CAPABILITIES DxgkDdiGetCapabilities;
    DXGKDDI_PANEL_BUFFER_CONTROL_SET_OVERRIDE_STATE DxgkDdiSetOverrideState;
    DXGKDDI_PANEL_BUFFER_CONTROL_GET_OVERRIDE_RESULT DxgkDdiGetOverrideResult;
    DXGKDDI_PANEL_BUFFER_CONTROL_SET_AUTOMATIC_POLICY DxgkDdiSetAutomaticPolicy;

} DXGK_PANEL_BUFFER_CONTROL_INTERFACE_1;
