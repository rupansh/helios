...
| **K9** | Completion ordering + interrupts: `DXGK_INTERRUPT_DMA_COMPLETED` only after exact host completion, one-engine ordered frontier with retained early completions, `DXGK_INTERRUPT_NATIVE_FENCE_SIGNALED`, reset/stale-callback generation invalidation. | `src/ddi/interrupt.rs`, `src/adapter/locks.rs` | K6, K7 | M | **NOT STARTED AS A FULL UNIT.** K7's narrow native-fence rescan seam now follows a successfully delivered WDDM submission and current monitored population through the existing DIRQL mechanism; ordered-engine/frontier work was not started. |
