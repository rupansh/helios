...
| **K5** | HTS1 translation sessions + HQA1 attach. `CreateProcess` allocates one bounded session list; `CreateDevice` records the exact `hKmdProcess`/adapter/package generation; HVC1 control creation makes a provisional session; INIT creates one host Venus context/`VkInstance` and returns generation+CSPRNG capability+endpoint capacity; D3D context create parses HQA1 once and stores a direct strong ref. | `src/ddi/translation_session.rs` (add), `src/device.rs` | K2+K4 (the role-1 HVM1 pool), protocol `translation_session.rs` | L | ⭐ **LANDED and EXERCISED ON THE TARGET** (KMD 22.22.264.0). Pure half: `kmd_logic::translation_session` (263 tests, was 211). Platform half: `ddi/translation_session.rs` + `device.rs`'s four-arm create-context dispatch. Gate: `tools/hts1-attach-gate.sh` (41 cases across the C mirror, defeated 5 ways). Acceptance: `tools/hts1_session_probe.c`, **15/15** on KMD 22.22.267.0 (cold boot). ⭐ The 15th was K4's HVM1 write-back not reaching user mode; root-caused and repaired in `a8527e2` — dxgkrnl DISCARDS a create-time write into `DXGK_ALLOCATIONINFO::pPrivateDriverData`, so the create-output is now stamped at `DxgkDdiOpenAllocation` (`FINDINGS.md` F11). `TsPoolBind=1`: the reply-pool binding fires. ⚠ Its INIT and HQA1-attach arms are **implemented and unreachable** until K6/K11; the counters say so at their declarations. See `ROADMAP.md` "K5 landed, and what it does NOT do". |
