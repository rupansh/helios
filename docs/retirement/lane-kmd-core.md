...
the original 5976-line frozen body of
`docs/HELIOS_PRESENT_SYNC_RETIREMENT.md` ("the doc" below) at the line ranges in
§1; append-only owner amendments follow that body without moving it. Where the
doc is silent or self-contradictory that is said plainly in §6 — do not invent
a reading; take the fail-closed one and record it.
...
**2026-08-14 K11 checkpoint.** D9 has raised `SURFACE` to
...
test. KMD 22.22.296.0 subsequently landed K11: each exact live HTS1 session owns
one distinct stock Venus context/`VkInstance` and one private 4 KiB host-reply
target, while the same K2a role-1 allocation remains the sole HVR1 carrier.
Finite INIT publishes generation/capability/bounded endpoint capacity only after
the actual host reply validates; allocation-backed and GPU-dependent operations
still refuse. Normal, abrupt, and held-reset teardown were exercised without a
stale session or stuck reply slot. Escape and HWQueues remain NULL; Mesa A3/A4
remain, and the display is runtime-unadmitted. See `FINDINGS.md` F18/F19. The
...
— note this is `grep`, where `\|` **is** alternation; the same string handed to
`rg` matches a literal pipe and returns nothing) and the cited text itself, not
by adding 10 on faith. Before citing a line, re-grep it.

⭐ **The other five lane briefs were swept on 2026-08-10 (round-3 repair), and
the sweep was verified mechanically** — every `§N.M`-anchored citation in
`docs/retirement/lane-*.md` was required to land inside §N.M's own
heading-to-heading range. That check found three citations in
`lane-vkd3d-umd12.md` and `lane-dxvk-umd11.md` that had *already* been corrected
on 2026-08-10 and would have been shifted twice; they are recorded in those
files' headers. ⛔ So the sentence above is now history, not an outstanding
defect — but the *method* stands: re-grep, never add 10.

**The frozen body is 5976 lines**: commit `afebe66` brought it to 5973 by
...
minimum (F1), the Code-43 segment wall does not exist (F2), and §10.3's
`ID3D12Fence` import is impossible and reverses direction (F8).
...
| `kmd_render/src/ddi/native_fence.rs` | ⭐ **EXISTS; DORMANT K7 COMPLETE.** Six callback implementations are registered. Exact per-adapter LUID/feature/lifecycle/generation authority, native caps, conditional scheduler-cap publication, bounded two-pass updates, direct-handle lifetime, and correlated native interrupt reporting are wired. The two native-log callbacks remain NULL/Disabled. `NATIVE_FENCE_ADVERTISED` still derives only from the 2.1 `SURFACE`, so none of this is production-active. (4318) |
...
| File | Lines | Today | Verdict |
|---|---:|---|---|
| `src/lib.rs` | run `wc -l` | ⛔⛔ **This cell has now been wrong twice, in opposite ways, and the second time was written by the commit titled "make the planning docs agree with the tree".** It said **6344 (was 5596)** with twenty module offsets labelled "re-measured 2026-08-10"; `wc -l` says **8144**, and `for c in 07364d6 c233fa9 2b63ada d803346 2e04189 HEAD; do git show $c:kmd_logic/src/lib.rs \| wc -l; done` returns 8144 for **every one of them** — so it was 8144 at the moment the "6344" was typed, and not one of the twenty offsets resolved. It also **omitted the two largest modules in the file.** ⇒ **No line figure and no offset table is stated here any more.** Get the inventory from the file, with `rg -n -e '^pub mod ' -e '^mod ' kmd_logic/src/lib.rs` (⛔ **`-e`, not `'^pub mod \|^mod '`** — `rg`'s `\|` is a literal pipe and that command returns nothing, which is almost certainly how the twenty bad offsets were produced). **The modules, by name, in file order:** `vsync_deadline`, the Venus `Writer`/encoders + its `tests`, `sorted_splice_tests`, `scanout_retire` (+tests), `scanout_lease` (+tests), `scanout_cadence` (+tests), `scanout_worker_bind` (+tests), `scanout_presentation_epoch` (+tests), `scanout_publish_txn` (+tests), `scanout_fast_bind` (+tests), `scanout_refresh` (+tests), **`scanout_read_ledger` + `scanout_read_ledger_tests`**, `snapshot_bind` (+tests), `windowed_blt_token` (+tests), `wddm_head_bound` (+tests), `wddm_boundary` (+tests), **`present_stream` + `present_stream_tests` + `present_stream_boundary_tests`**, `native_fence_lifecycle` + tests (added by commit `476f9ce`, i.e. K13 is partly landed), and ⭐ **`allocation_identity` + `allocation_identity_tests` — the K4 model, ~856 lines of model and ~948 of tests, which appeared in NO retirement doc until 2026-08-10.** Test count: `grep -c '#\[test\]' kmd_logic/src/lib.rs` = **211** at HEAD (was 189, was 172 — regenerate, do not cite; §5), **25 of them in `allocation_identity_tests`**. ⛔ Read `K4-CONTRACT.md` §8 before crediting those 25: `kmd_render` calls **zero** functions from `kmd_logic::allocation_identity`, so they grade a parallel model, not the shipped decision. | **MODIFY** (4315). Delete `scanout_read_ledger*` and `present_stream*` (A.2 row 5721, A.2 row 5734). Add executable models per §17.6:4315 "exact batch/queue/flip tests" and A.2 row 5721: per-context submission completion, Core-0116 native-fence lifecycle, plane candidate/current/backend-release, reset, mode transition, destruction, multi-queue/swapchain. **Plus the outstanding unit in `K4-CONTRACT.md` §8: wire `kmd_render` to call `allocation_identity` instead of hand-writing a second copy.** |
| `Cargo.toml` | run `wc -l` | ⛔ **STALE — corrected 2026-08-10.** This row said "Deliberately dependency-free (no `wdk-sys`, no `helios_protocol`)". **K4 added the `helios_protocol` edge** (`helios_protocol = { path = "../protocol" }`), with the argument written out in the manifest itself: the invariant the dependency list protects is *not* "no dependencies", it is "nothing reachable from here may read kernel state" — no `wdk-sys`, no generated `dxgk`/`ntddk` bindings, no `kmd_render` types. `helios_protocol` is `#![no_std]`, is already a `kmd_render` dependency, and declares wire records that carry no handle and no pointer by construction, so it satisfies that invariant. The manifest also records the counter-example: `native_fence_lifecycle` re-declares the whole HNF1 offset table locally *because* this edge was absent, which is a second declaration of a wire record and exactly what the "shared private data has ONE declaration" directive forbids. | **DECIDED, done.** The §6 "decision point" is closed in favour of taking the edge. `bytemuck` comes in transitively and is **not** re-exported by `helios_protocol`, so it is not nameable from `kmd_logic`. |
...
| **K4** | Allocation object model: HWA2 create-time descriptor, HVM1 roles 1–4, HOC1, and the deletion of UMD-backing adoption + open-time restamping. ⭐ **`docs/retirement/K4-CONTRACT.md` is normative for this unit and wins over this row wherever they differ.** | `src/ddi/create_allocation.rs`, `src/adapter/allocation_object.rs` **(add)**, `src/adapter/tracking.rs` (del). ⛔ **`src/adapter/kobj.rs` REMOVED from this set** — it is kernel dispatcher objects, not allocation state (§2.3). ⛔ **`src/adapter/backing.rs` REMOVED** — K4 owned 1 of its non-definition references and has now removed it, leaving **16 references, none of them K4's** (§2.3 has the per-file re-measurement); the file's deletion is re-sequenced to K3 (§2.3, §4). ⚠ `create_allocation.rs` is a single-owner file (`OWNERSHIP.md §1`): renaming or deleting its scanout/present accessor family breaks `ddi/display.rs` at compile time, in a build only the VM can run. | K2 (segments/placement) — but see K4-CONTRACT §4: **K2 is RESCOPED, not blocked** (HPM1 was declined, not deferred; F5's Consequence is that no lane in flight has a QEMU dependency), so K4 does not wait on it. ⛔ **This cell said K4 "refuses role-4 placement with a named counter until K2 lands" — that is the WITHDRAWN draft of §4 and it is wrong in both directions.** The ruling is **check the segment, do not hardcode the role**: K4 records `HELIOS_SEGMENT_ID_HLM1`, verifies at runtime that the role's `preferred_segment` is in the table the driver actually reports, and refuses **per role** with `AcSegRole1`…`AcSegRole4` when it is not. Protocol `wddm.rs` + `native_render.rs` are complete and asserted today. | XL | ⭐ **AUTHORED, ALL FIVE COMPONENTS BUILD, NOTHING HAS RUN.** (This cell said "NOT STARTED … `create_allocation.rs` unchanged at 3411 lines" through the changeset that landed it.) Now: `create_allocation.rs` grew by ~1200 lines (**do not cite a figure — it was still being edited while this cell was written**; run `wc -l`); `adapter/allocation_object.rs` added; `adapter/tracking.rs` deleted; `fn segment_is_reported` plus the four per-role counters `AcSegRole1`…`AcSegRole4` (`rg -n -e segment_is_reported -e AcSegRole kmd_render/src/` (⛔ **`-e`, not `A\|B`** — a raw pipe here also split this markdown cell, and the escaped `A\|B` form `rg` would then see matches a LITERAL pipe and returns nothing; `-e` is the only form that is both legal in a table and correct in `rg`)). Verified by `tools/retirement-gates.sh` **8/8 PASS** — including §8.5 (open writes no byte of the private buffer) and §8.6 (the retired identity symbols have zero live occurrences; the gate prints its own symbol and tombstone counts, which move as it is strengthened). ⛔ Every HWA2 path is **implemented but never exercised**, `METHOD.md`'s distinct third state. |
| **K5** | HTS1 translation sessions + HQA1 attach. `CreateProcess` allocates one bounded session list; `CreateDevice` records the exact `hKmdProcess`/adapter/package generation; HVC1 control creation makes a provisional session; INIT returns generation+CSPRNG capability+endpoint capacity only after K11 host success; D3D context create parses HQA1 once and stores a direct strong ref. | `src/ddi/translation_session.rs` (add), `src/device.rs` | K2a+K4 (the role-1 HVM1 pool), protocol `translation_session.rs` | L | ⭐ **LANDED, HOST-REACHABLE, AND EXERCISED ON THE TARGET.** The K5 lifetime/attach model remains authoritative; K11 supplies only its exact host boundary. `tools/hts1_session_probe.c` passes **15/15** on KMD 22.22.296.0, and the K11 probe reaches live HQA1 validation with simultaneous distinct sessions. The old unreachable-until-K6/K11 note is historical. |
| **K6** | HVC1 legacy contexts + HNR2 Render/Patch/SubmitCommand + context-local slot and WDDM submission ownership. | `src/ddi/native_render.rs` (add), `src/ddi/submit_command.rs`, `src/ddi/scheduler.rs` | K4, K5; protocol `native_render.rs`+`wddm.rs` | XL | ⭐ **LANDED AND EXERCISED ON THE TARGET.** K6 supplies the real HNR2 decoder, fragment assembly, exact role-1 slot admission, repeatable Patch, and physical SubmitCommand arm. K11 closes only the finite pure-control INIT handoff and records its actual host-completed WDDM submission; allocation-backed, queue, residency, general-schema, and GPU work still refuse at named boundaries. The updated `tools/hnr2_native_probe.c` passes **15/15** on KMD 22.22.296.0, including a same-session repeated INIT refusal with `TsSlotStuck=0`. |
| **K7** | Native-fence DDI surface: `DxgkDdiCreateNativeFence` / `OpenNativeFence` / `CloseNativeFence` / `DestroyNativeFence` (+ `SetNativeFenceLogBuffer` / `UpdateNativeFenceLogs` classified), `DXGKQAITYPE_NATIVE_FENCE_CAPS`, `DXGK_FEATURE_NATIVE_FENCE` enablement, `DXGK_VIDSCHCAPS::NativeGpuFence=1`, `No64BitAtomics=0`, HNF1 PDD validation. Bounded objects referenced directly by driver global/local handles — no table, no scan, no name. | `src/ddi/native_fence.rs` (add), plus its narrow query/interrupt/lifecycle seams | K0 (table slots); UMD12 lane consumes the other side | L | ✅ **DORMANT TRANCHE COMPLETE; PRODUCTION AUTHORITY ABSENT.** Exact StartDevice LUID publication, per-adapter stable feature admission, native-caps query, conjunctive driver-cap publication, six registered callbacks, two-pass bounded updates, nonwrapping generations, direct-handle lifetime, correlated rescan interrupt, and existing reset/stop/remove/skipped-stop invalidation now compose. NF-UAF-1 is closed by the documented dxgkrnl reference order plus a bounded fail-closed outstanding-local refusal. K7's real gate imports D4/D5 and rejects 35 in-memory mutations. Windows check is green at 13 warnings, all 518 logic tests plus both integrations pass, and the WDK-28000 binding hash agrees. Both activation switches remain false; there is no runtime or activation-readiness claim. |
| **K8** | Adapter caps + query surface: `query_adapter_info.rs` end state (driver caps, WDDM device caps, node metadata, native-fence caps, physical-memory caps, the segment writers). | `src/ddi/query_adapter_info.rs` | K2, K7 | L | **NOT STARTED AS A FULL UNIT.** K7's narrowly owned native-fence caps arm and conditional `NativeGpuFence` bit are now present; physical-memory caps, segment end state, and the rest of K8 were not started. |
| **K9** | Completion ordering + interrupts: `DXGK_INTERRUPT_DMA_COMPLETED` only after exact host completion, one-engine ordered frontier with retained early completions, `DXGK_INTERRUPT_NATIVE_FENCE_SIGNALED`, reset/stale-callback generation invalidation. | `src/ddi/interrupt.rs`, `src/adapter/locks.rs` | K6, K7 | M | **SOURCE/BUILD-VALIDATED; TARGET-RUNTIME UNEXERCISED.** One fixed 256-entry frontier is owned per adapter and admits the exact scheduler fence in callback-arrival order. Direct K11 and compatibility Venus work retain generation/serial/slot tickets; host completion marks only the exact ticket, early cross-context results remain resident, and only a contiguous ready head reaches `DXGK_INTERRUPT_DMA_COMPLETED`. Callback failure retains the head for DPC retry; transport refusal/FIFO exhaustion poison rather than synthesize completion. Reset, preemption, Stop/Remove, and stale callbacks use generation invalidation, with reopen only after an accepted preemption, completed ResetEngine recovery, or a complete successor transport. K7 native-fence rescans remain downstream of successfully delivered DMA edges. The K9 gate imports K11 and rejects **42** in-memory mutations; Windows compilation remains at the exact 13-warning baseline. This is the ordered-completion prerequisite identified by A4; it has not been deployed, adapter-restarted, or exercised on the target, and no display-admission claim follows. |
| **K10** | Adapter/device lifecycle: StartDevice LUID + HPM1 ordering, StopDevice/RemoveDevice drain order, reset generation transitions, deletion of `bring_up_venus`. | `src/ddi/lifecycle.rs`, `src/adapter/mod.rs` | K2, K5, K6 | L | **NOT STARTED AS A FULL UNIT.** K7 now captures the exact StartDevice LUID and owns native-fence reset/stop/remove generation transitions while preserving the existing invalidation order. HPM1 ordering, `bring_up_venus` deletion, and the rest of K10 were not started. |
| **K11** | Escape-free per-session host transport for the finite pure-control INIT: one stock Venus context/object namespace and private host-reply target per exact HTS1 session; actual host reply through role-1 HVR1; bounded endpoint/ring reservation; direct outer refs; exact rundown and context-local WDDM submission watermark. No allocation/GPU queue executor in this tranche. | `src/ddi/session_transport.rs` (add), narrow seams in `translation_session.rs`, `native_render.rs`, `submit_command.rs`, `device.rs`, `create_allocation.rs`, `src/virtio/control_owner.rs`, and `src/virtio/ctrl.rs` | K2a, K5, K6; existing stock virtio-gpu/Venus only | XL | ⭐ **LANDED AND EXERCISED ON THE TARGET.** KMD 22.22.296.0 / `oem128.inf`: K11 **14/14**, HTS1 **15/15**, updated HNR2 **15/15**, K2a **52/52**; distinct same-process sessions, cross-process isolation, repeated and abrupt teardown, exact correlated HVR1, and held-capability adapter restart all passed. Post-reset fresh run: `K11CtxNew=K11CtxDel=8`, all K11 failure counters zero, `K11CmpOpenRej=0`, and `TsSlotStuck=0`. The final fixed per-adapter completion rundown closes the review-found reset/notification deadlock without a queue or lookup. The semantic gate rejects 62 mutations. No QEMU/virglrenderer/HPM1/kernel-parameter change, no Escape/HWQueue, and no display-admission claim. Mesa A3/A4 is the handoff. |
...
* K10 and K2 both touch `lifecycle.rs::dxgkddi_start_device`. K2 adds the HPM1
  ...
  serially.
* K11 is the only completed unit in this tranche that touches `virtio/`. Its
  narrow stock-context/control-owner seams have landed; later work must preserve
  the exact per-session ownership and must not fold it into adapter-global state.
...
| `src/virtio/ctrl.rs` | This lane (K11). The legacy `read_ledger.note_alloc_retired` calls remain K1-owned. | Ordering. | K11 landed only the bounded context/resource pair and direct session-operation seams. K1 removes the unrelated read-ledger calls later without weakening those seams. |
...
| `src/virtio/venus/scanout.rs`, `venus/present.rs` | Display lane's scanout path runs through these. | K11's per-session transport deliberately did not rewrite these adapter display paths. | Preserve that separation: later display/Mesa work must not treat the adapter `VenusClient` as the HTS1 session namespace or add discovery between them. |
...
`rg -n -e alloc -e generation -e HWA2 -e HVM1 -e segment kmd_render/src/adapter/kobj.rs`
...
⭐ **CLOSED 2026-08-10 — the recommendation was taken, explicitly.** `kmd_logic`
now depends on `helios_protocol = { path = "../protocol" }`, and the manifest
comment was rewritten to name the actual invariant in exactly those terms
("nothing reachable from here may read kernel state … no `wdk-sys`, no
`wdk-build`, no generated `dxgk`/`ntddk` bindings, and no `kmd_render` types").
The manifest also records the counter-example that settled it:
`native_fence_lifecycle` re-declares the whole HNF1 offset table locally
*because* the edge was absent — a second declaration of a wire record, which the
"shared private data has ONE declaration" directive forbids. ⚠ Two consequences
worth carrying forward: (a) `bytemuck` enters the graph transitively but is not
re-exported by `helios_protocol`, so it stays un-nameable from `kmd_logic`; and
(b) the edge is what makes `K4-CONTRACT.md` §8 row 10 cheap — the models already
speak `protocol`'s record types, so wiring `kmd_render` to call
`kmd_logic::allocation_identity` is a call-site change, not a redesign.
