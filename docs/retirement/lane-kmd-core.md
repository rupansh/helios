...
| **K4** | Allocation object model: HWA2 create-time descriptor, HVM1 roles 1–4, HOC1, and the deletion of UMD-backing adoption + open-time restamping. ⭐ **`docs/retirement/K4-CONTRACT.md` is normative for this unit and wins over this row wherever they differ.** | `src/ddi/create_allocation.rs`, `src/adapter/allocation_object.rs` **(add)**, `src/adapter/tracking.rs` (del). ⛔ **`src/adapter/kobj.rs` REMOVED from this set** — it is kernel dispatcher objects, not allocation state (§2.3). ⛔ **`src/adapter/backing.rs` REMOVED** — K4 owns 1 of its 14 non-definition references; the file's deletion is re-sequenced to K3 (§2.3, §4). ⚠ `create_allocation.rs` is a single-owner file (`OWNERSHIP.md §1`): renaming or deleting its scanout/present accessor family breaks `ddi/display.rs` at compile time, in a build only the VM can run. | K2 (segments/placement) — but see K4-CONTRACT §4: **K2's host counterpart is parked (F5), so K4 does not wait on it.** K4 records `HELIOS_SEGMENT_ID_HLM1` and refuses role-4 placement with a named counter until K2 lands. Protocol `wddm.rs` + `native_render.rs` are complete and asserted today. | XL | **NOT STARTED.** `rg 'HeliosWddmAllocationDescV2\|HELIOS_HWA2\|HeliosVenusMemoryAllocationV1\|HeliosOuterCommandAllocationV1\|HVM1\|HOC1\|HWA2' kmd_render/src/` returns **nothing** outside a doc string in `wddm32_slot_audit.rs`; `create_allocation.rs` unchanged at 3411 lines and `git log d1c820a..HEAD -- create_allocation.rs tracking.rs backing.rs` is empty. K4 is the first KMD consumer of all three records. |
...
* `adapter/backing.rs` is **K3's** deletion, not K4's: K3 owns 10 of the 14
  non-definition references, the display lane owns 2, `adapter/mod.rs` 3 and K4
  exactly 1 (`create_allocation.rs:1836`). K4 removes its one line; K3 removes
  the file. See §4 for the display-lane request that unblocks it.
