...
verified.** The doc's frozen body reached **5976 lines** — 5918 when this brief
was written, 5928 after commit `c17f17c` inserted a ten-line banner at `:11-20`,
and 5973 after `afebe66` **appended** the SUPERSEDED CLAIMS INDEX at `:5932`;
later append-only index corrections raised the frozen total to 5976 and newer
checkpoint amendments follow without moving a body line. Only the banner moved
anything: it shifted every body line after 8 by +10, so every
...
**2026-08-21 host/packaging source-demolition checkpoint.** QEMU and
virglrenderer remain unchanged; K2a is still the only scoped host-side exception
and HPM1 remains parked. Root `5901f55` removes obsolete probes and every
source-side creator, ACL grant, verifier assumption, and install-state carrier
for `C:\ProgramData\Helios\helios_present_sync_v2.bin`; it performs no deletion
against a live machine. K14 `dcf0e1e` advances source generation once to 4 and
KMD version once to 22.22.297.0, requires a complete generation-matched lower/
layer/both-UMD payload, source-wires the generated layer manifest and exact DLL
path, and retains translator-owned A5 bypass. Parser, in-memory package-source,
negative-manifest, focused, and serial gates pass. Package assembly, installer/
verifier/uninstaller execution, ProgramData cleanup, registry mutation, layer
registration, driver staging, and every target exercise were deliberately not
performed.
