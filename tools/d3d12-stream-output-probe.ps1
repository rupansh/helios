...
param(
    ...
    [string]$BuildDir = 'C:\ProgramData\Helios\stream-output-probe',
    [string]$ArchiveRoot = 'Z:\tmp\fl12-stream-output',
...
$localRunner = Join-Path $BuildDir 'd3d12-stream-output-probe.ps1'
$provenancePath = Join-Path $BuildDir 'build-provenance.json'
$inputNames = @('d3d12_stream_output_probe.cpp','d3d12_stream_output_probe.hlsl',
    'd3d12_native_identity.h','d3d12-stream-output-probe.ps1')
$artifactNames = @('stream-output-vs.dxil','stream-output-gs.dxil','stream-output-ps.dxil',
    'd3d12_stream_output_probe.exe','build.cmd','build.log')
...
function Assert-Build {
    ...
    if ($build.Schema -ne 1 -or $build.Probe -ne 'native-stream-output') { throw 'Invalid SO build receipt.' }
    foreach ($category in @('Inputs','Artifacts')) {
        $names = if ($category -eq 'Inputs') { $inputNames } else { $artifactNames }
        $entries = @($build.$category)
        if ($entries.Count -ne $names.Count) { throw "Incomplete or extra $category in SO build receipt." }
    ...
    $runner = @($build.Inputs | Where-Object { $_.Name -eq 'd3d12-stream-output-probe.ps1' })[0]
...
if ($Mode -eq 'Build') {
    ...
    try {
        ...
        try {
            & $dxc -T vs_6_0 -E vs_main -Fo stream-output-vs.dxil d3d12_stream_output_probe.hlsl
            if ($LASTEXITCODE) { throw "DXC VS failed: $LASTEXITCODE" }
            & $dxc -T gs_6_0 -E gs_main -Fo stream-output-gs.dxil d3d12_stream_output_probe.hlsl
            if ($LASTEXITCODE) { throw "DXC GS failed: $LASTEXITCODE" }
            & $dxc -T ps_6_0 -E ps_main -Fo stream-output-ps.dxil d3d12_stream_output_probe.hlsl
...
set VCToolsVersion
cl /nologo /Bv /W4 /EHsc /std:c++17 /O2 d3d12_stream_output_probe.cpp /Fe:d3d12_stream_output_probe.exe /link d3d12.lib dxgi.lib 2>&1
exit /b %errorlevel%
"@
        ...
        $provenance = [ordered]@{ Schema = 1; Probe = 'native-stream-output'; UTC = [DateTime]::UtcNow.ToString('o')
    ...
    Write-Output "Built native stream-output probe at $BuildDir"
    exit 0
}
...
if ($Mode -eq 'Schedule') {
    $null = Assert-Build
    Assert-NoOverrides
    $taskName = 'helios_stream_output_acceptance'
...
$result = [ordered]@{ UTC = [DateTime]::UtcNow.ToString('o'); Session = (Get-Process -Id $PID).SessionId
    ...
    Scope = 'Bounded native SO behavior; no feature-level, benchmark, visual or performance acceptance.' }
...
try {
    ...
    $result.LocalRunner = Get-Artifact 'd3d12-stream-output-probe.ps1'
    ...
    $info.FileName = Join-Path $BuildDir 'd3d12_stream_output_probe.exe'
    ...
    if ($process.ExitCode -ne 0 -or !$outText.Contains('PASS,stream-output,34 cases') -or
            !$outText.Contains('PASS,negative-root-allow-stream-output,') -or
            !$outText.Contains('PASS,null-so-array,count=1,same-stream-stopped,written=0,needed=1')) {
        throw 'Native SO acceptance failed; inspect archived output.'
    }
    foreach ($streams in @(1,2)) { foreach ($stride in @(4,16,32)) { foreach ($caseMode in 0..3) {
        if (!$outText.Contains("PASS,null-so-contract,streams=$streams,stride=$stride,mode=$caseMode,counter32-heap-end")) {
            throw "Missing NULL/zero-size/overflow/counter boundary case: streams=$streams stride=$stride mode=$caseMode"
        }
    } } }
