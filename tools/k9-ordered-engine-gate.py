...
def check_admission_and_completion(
    ...
    require_order(
        ...
        dpc,
        (
            "transport.take_one_ready_wddm(order)",
...
def mutation_cases() -> tuple[Mutation, ...]:
    return (
        ...
        Mutation("split compatibility bound", GPU, "const MAX_WDDM_PENDING: usize = helios_kmd_logic::ordered_engine::MAX_ORDERED_ENGINE_SUBMISSIONS;", "const MAX_WDDM_PENDING: usize = 512;"),
        Mutation("restore scalar WDDM fence", GPU, "struct WddmPending {\n", "struct WddmPending {\n    fence: u32,\n"),
        Mutation("complete transport failure", GPU, "            self.wddm_pending.clear();\n            return WddmAdmission::Failed;", "            self.wddm_pending.clear();\n            return WddmAdmission::HostTerminal;"),
        Mutation("complete compatibility overflow", GPU, "            self.overflow_wddm_pending();\n            return WddmAdmission::Failed;", "            self.overflow_wddm_pending();\n            return WddmAdmission::HostTerminal;"),
        ...
        Mutation("drop undelivered compatibility owner", INTERRUPT, "            let _ =\n                guard.with_virtio(|order, transport| transport.requeue_wddm_front(order, ready));", "            ready.delivered();"),
