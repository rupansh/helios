...
param(
    ...
    [string]$BuildDir = 'C:\ProgramData\Helios\allocator-probe',
    [string]$ArchiveRoot = 'Z:\tmp\fl12-allocator',
...
$localRunner = Join-Path $BuildDir 'd3d12-allocator-probe.ps1'
$receiptPath = Join-Path $BuildDir 'build-provenance.json'
$sourceNames = @('d3d12_allocator_probe.cpp',
                 'd3d12_native_identity.h','d3d12-allocator-probe.ps1')
$artifactNames = $sourceNames + @('d3d12_allocator_probe.exe','build.cmd','build.log','cl-path.txt')
...
function Assert-Build([ref]$VerifiedReceiptBytes) {
    ...
    $recordedRunner = @($receipt.Artifacts | Where-Object { $_.Name -ceq 'd3d12-allocator-probe.ps1' })[0]
...
if ($Mode -eq 'Build') {
    ...
    try {
        $build = "call `"$vs\VC\Auxiliary\Build\vcvars64.bat`"`r`nwhere cl > cl-path.txt`r`ncl /nologo /W4 /WX /EHsc /std:c++17 /O2 d3d12_allocator_probe.cpp /Fe:d3d12_allocator_probe.exe /link d3d12.lib dxgi.lib`r`nexit /b %errorlevel%`r`n"
    ...
    Write-Output "Built native allocator probe at $BuildDir"
    exit 0
}
if ($Mode -eq 'Schedule') {
    $previous = Get-ScheduledTask -TaskName 'helios_allocator_acceptance' -ErrorAction SilentlyContinue
    if ($previous -and $previous.State -eq 'Running') { throw 'Previous allocator probe is still running; preserve its task and archive.' }
    ...
    Register-ScheduledTask -TaskName 'helios_allocator_acceptance' -Action $action -Principal $principal -Settings $settings -Force | Out-Null
    Start-ScheduledTask -TaskName 'helios_allocator_acceptance'
    Write-Output 'Started interactive helios_allocator_acceptance task.'
    exit 0
}
...
try {
    ...
    $info.FileName = "$runDir\d3d12_allocator_probe.exe"
    $info.Arguments = ''
    ...
    $required = @('PASS,allocator_reuse,type=0,epochs=256,words=4096',
        'PASS,allocator_reuse,type=2,epochs=256,words=4096', 'PASS,allocator_reuse,type=3,epochs=256,words=4096',
        'PASS,shared_allocator_two_queues,epochs=64', 'PASS,pending_list_reset_allocator_release',
        'PASS,native_allocator_probe_completed')
    ...
    if ($umd.Count -gt 1 -or (($admitted -or $workloadPassed) -and $umd.Count -ne 1)) {
        throw 'Admitted native allocator requires exactly one identified Helios UMD module.'
    }
    ...
    if ($blocked) {
        ...
        throw 'Native allocator probe failed; inspect archived output. No automatic retry.'
    } else {
