...
- `tools/d3d11_msaa_present_probe.cpp` — native-runtime x86/x64 legacy DISCARD presentation, 1x/4x UNORM/sRGB. Checks every resolved RGBA pixel against a CPU pattern, including sample-frequency shading and sRGB midtones; writes actual/reference BMPs. Append `resize` after the duration (at least 12 seconds) to exercise twelve geometries in one device. Build with `tools/build-d3d11-msaa-present-probe.ps1`, run interactively, and compare host VNC frames separately. Source readback alone does not pass presentation; .273 passes readback while visibly corrupting 4x sRGB and mishandling the sample/midtone bands (ROADMAP WoW64 section).
