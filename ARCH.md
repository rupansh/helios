...
**NEXT = Phase 5 (the Mesa venus ICD, §5)** — the real validator of the blob path (a host-visible blob needs a venus `vkAllocateMemory` backing the host won't give a standalone probe). **Mesa is now a git submodule at `icd/mesa`** (upstream main `4e8595d`; repoint `.gitmodules` to our fork once it exists — "we'll maintain a fork"). Next sub-steps: baseline `-Dvulkan-drivers=virtio` meson configure on win11 (capture libdrm/meson gates), then write `icd/mesa/src/virtio/vulkan/vn_renderer_helios.c` (model on `vn_renderer_vtest.c`, NOT virtgpu) — this is MAX/ULTRAC-effort, workflow-driven. Deferred: the WDFINTERRUPT + async-fence DPC (resolve the `STATUS_DEVICE_POWER_FAILURE` legacy-INTx question then). Detail: the `phase4-blob-plan` and `mesa-venus-icd-port` memories.
