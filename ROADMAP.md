...
| Component | State | Why |
|---|---|---|
| `icd/mesa/.../helios-present-layer/` | **BUILDS** — `-Dvulkan-layers=helios-present` produces `VkLayer_HELIOS_present.dll` + manifest | Not **installed**: packaging and loader registration are a cross-lane request against `packaging/windows/Install-Helios.ps1` (`OWNERSHIP.md` §1), so nothing loads it yet. It had never been compiled at all; first contact found **17 errors** of self-drift, one gate that would have been permanently dark, and — only from the real mingw build — a silent DLL-name mismatch (`REVIEW-ROUND-1.md` review 4). A `.def` is **not** needed: all eight loader entry points are exported, verified with `objdump -p` on the built DLL |
