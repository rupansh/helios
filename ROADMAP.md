...
| 2 | **D2** `direct_scanout.rs` (ADD) | D3 depends on it | **PURE OWNERSHIP MODEL LANDED; KMD D2 ABSENT.** `kmd_logic::direct_scanout_lifetime` models move-only real/parking backing ownership, exact fenced completion, reset/removal, and stale/replay refusal. Platform work still owes the strong backing reference, committed-mode validator, and transport-wide fence mint/callback rendezvous. §6 items 6 and 8 are resolved with immutable `qemu-helios` and parking-first unbind. |
