...
| 2 | **D2** `direct_scanout.rs` (ADD) | D3 depends on it | **PURE ADMISSION, PLANE, BACKING, AND CONTROL-OWNERSHIP MODELS LANDED; KMD D2 ABSENT.** `kmd_logic` now models the exact final-HWA2/source/committed-mode/plane predicate, move-only real/parking lifetime, atomic display-backing references, and domain/epoch-qualified resource-window control outcomes. KMD has reset-safe committed-mode storage, exact CommitVidPn fact extraction, and a waiter disposition that distinguishes a copied host response from transport abort; none is yet a D2 authority. Platform work still owes the pointer-bound transport owner, strong allocation-backing split plus PASSIVE finalizer, atomic conversion of every ownership-sensitive control caller, coherent mode/plane publication, and transport-wide fence mint/callback rendezvous. The normative LINEAR profile remains intentionally unwired while today's OPAQUE producer disagrees. §6 items 6 and 8 are resolved with immutable `qemu-helios` and parking-first unbind. |
