...
| 2 | **D2** `direct_scanout.rs` (ADD) | D3 depends on it | **PURE D2 OWNERSHIP GRAPH AND THE DORMANT KMD CONTROL-OWNER FOUNDATION LANDED; KMD D2 ABSENT.** `kmd_logic` now models the exact final-HWA2/source/committed-mode/plane predicate, move-only real/parking lifetime, atomic display-backing references, domain/epoch-qualified resource-window control outcomes, canonical `(context, resource)` associations, and an exact attachment-bound context lease/census that blocks context destruction and resource unref. KMD has reset-safe committed-mode storage, exact CommitVidPn fact extraction, lossless post-ticket control observations, and one driver-unique control domain bound once during adapter construction; none mints a production reservation or D2 authority. Activation remains one atomic platform boundary: the exact-address generation gateway and canonical tables; typed CREATE/ATTACH/DETACH/UNREF/MAP/UNMAP/CTX_CREATE/CTX_DESTROY callers; the strong allocation-backing split and dedicated PASSIVE finalizer; a verified physical transport reset before any reset witness; coherent mode/plane publication; and the transport-wide fence/callback rendezvous. The normative LINEAR profile remains intentionally unwired while today's OPAQUE producer disagrees. §6 items 6 and 8 are resolved with immutable `qemu-helios` and parking-first unbind. |
