...
| 2 | **D2** `direct_scanout.rs` (ADD) | D3 depends on it | **PURE ADMISSION + OWNERSHIP MODELS LANDED; KMD D2 ABSENT.** `kmd_logic` now models the exact final-HWA2/source/committed-mode/plane predicate and move-only real/parking lifetime, including fenced completion, reset/removal, and stale/replay refusal. Platform work still owes the strong backing reference, coherent committed-mode publication, and transport-wide fence mint/callback rendezvous. The normative LINEAR profile remains intentionally unwired while today's OPAQUE producer disagrees. §6 items 6 and 8 are resolved with immutable `qemu-helios` and parking-first unbind. |
